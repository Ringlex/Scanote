...
class Application extends StatelessWidget {
  Application({required this.appTheme, required this.appRouteFactory, this.initialThemeMode, super.key});
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (context) => injector<AuthBloc>()..add(const AuthEvent.onInitiated())),
        BlocProvider(create: (context) => injector<LocaleBloc>()..add(const LocaleEvent.onInitiated())),
      ],
      child: AdaptiveTheme(
        ...
        builder: (theme, darkTheme) => BlocBuilder<LocaleBloc, LocaleState>(
          builder: (context, localeState) => MaterialApp.router(
            ...
            builder: (context, child) => NavigationHub(rootNavigatorKey: _rootNavigatorKey, child: child),
          ),
        ),
      ),
    );
